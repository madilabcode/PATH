...
class SlideNumberDiscriminator(nn.Module):
    def __init__(self, input_dim, num_slides, num_samples, num_datasets):
        super(SlideNumberDiscriminator, self).__init__()
        ...
        self.model = nn.Sequential(
            ...
            nn.Linear(input_dim // 8, 250),
            nn.LeakyReLU(0.2),
        )

        self.slide_number_classifier = nn.Linear(250, num_slides)
        self.sample_number_classifier = nn.Linear(250, num_samples)
        self.dataset_classifier = nn.Linear(250, num_datasets)
        
    def forward(self, x):
        embeddings = self.model(x)
        slide_number_logits = self.slide_number_classifier(embeddings)
        sample_number_logits = self.sample_number_classifier(embeddings)
        dataset_logits = self.dataset_classifier(embeddings)
        return slide_number_logits, sample_number_logits, dataset_logits
...
class GeneExpressionPredictor(nn.Module):
    ...
    def __init__(
        ...
        model_path: str = '/home/bnet/ronsheinin/spNET/TransPath/ctranspath.pth',
        ...
        self.slide_number_discriminator = SlideNumberDiscriminator(embedding_dim, self.num_slides, self.num_samples, self.num_datasets)
    ...
    def fine_tune_on_spesific_pathway(self, idex: int, train_loader: DataLoader, val_loader: DataLoader, num_epochs: int = 10, learning_rate: float = 1e-4, train_all_pathways: bool = False) -> float: 
        """
        Fine-tune the model on a specific pathway.
        """
        self.train()
        self.eval()
        self.backbone.eval()
        self.kegg_head.train()
        self.slide_number_discriminator.train()

        device = self.device
        optimizer = optim.Adam(self.parameters(), lr=learning_rate)

        if train_all_pathways:
            loss_fn = Loss.AUCMarginLoss(margin=1.0, reduction='mean')
        else:
            loss_fn = Loss.AUCMarginLoss(margin=1.0, reduction='None')
        ...
        for epoch in range(num_epochs):
            ...
            for i, (images,kegg_expressions, slide_number, sample_number, dataset_number) in enumerate(train_pbar):
                ...
                slide_number = slide_number.to(device)
                sample_number = sample_number.to(device)
                dataset_number = dataset_number.to(device)
                ...
                if train_all_pathways:
                    ...
                    kegg_loss = loss_fn(kegg_predictions.float()[:,idex], kegg_expressions.float()[:,idex])
                kegg_loss.backward()
                optimizer.step()
                train_pbar.set_postfix({"kegg_loss": f"{kegg_loss.item():.4f}"})
                all_train_predictions.append(kegg_predictions.float()[:,idex].detach().cpu().numpy())
                all_train_targets.append(kegg_expressions.float()[:,idex].detach().cpu().numpy())
            ...
            train_roc_auc = roc_auc_score(all_train_targets, all_train_predictions)
            print(f"Epoch {epoch+1}/{num_epochs} - Train ROC-AUC: {train_roc_auc:.4f}")

            all_predictions = []
            all_targets = []
            val_pbar = tqdm(val_loader, desc=f"Epoch {epoch+1}/{num_epochs} [Val]")
            for i, (images,kegg_expressions, slide_number, sample_number, dataset_number) in enumerate(val_pbar):
                images = images.to(device)
                slide_number = slide_number.to(device)
                sample_number = sample_number.to(device)
                dataset_number = dataset_number.to(device)
                ...
                with torch.no_grad():
                    ...
                    if train_all_pathways:
                        ...
                        kegg_loss = loss_fn(kegg_predictions.float()[:,idex], kegg_expressions.float()[:,idex])
                        all_predictions.append(kegg_predictions.float()[:,idex].cpu().numpy())
                        all_targets.append(kegg_expressions.float()[:,idex].cpu().numpy())
    ...
    def train_withno_adversarial(self, train_loader: DataLoader, val_loader: DataLoader, num_epochs: int = 10, learning_rate: float = 1e-4, freeze_backbone: bool = False, auc_training: bool = True) -> float:
        ...
        for epoch in range(num_epochs):
            ...
            all_train_predictions = []
            all_train_targets = []
            for i, (images,kegg_expressions) in enumerate(train_pbar):
                images = images.to(device)
                kegg_expressions = kegg_expressions.to(device)
            ...
            self.kegg_head.eval()
            self.slide_number_discriminator.eval()
            for i, (images,kegg_expressions) in enumerate(val_pbar):
                images = images.to(device)
                kegg_expressions = kegg_expressions.to(device)
    ...
    def train_model(
        ...
        if self.classification_mode and not auc_training:
            #loss_fn_kegg = nn.BCEWithLogitsLoss(reduction='mean', pos_weight=torch.tensor([20]).to(self.device))
        ...
        for epoch in range(num_epochs):
            ...
            for i, (images,kegg_expressions, slide_number, sample_number, dataset_number) in enumerate(train_pbar):
                ...
                # =======================
                self.eval()  # freeze BN/dropout behaviour for backbone if you want
                ...
                disc_loss = disc_criterion(slide_number_logits, slide_number.long()) + disc_criterion(sample_number_logits, sample_number.long()) + disc_criterion(dataset_number_logits, dataset_number.long())
                ...
                disc_loss_adv = disc_criterion(slide_number_logits_adv, slide_number.long()) + disc_criterion(sample_number_logits_adv, sample_number.long()) + disc_criterion(dataset_number_logits_adv, dataset_number.long())
                disc_loss_adv = disc_loss_adv / 3.0

                if epoch >= self.num_epochs_adv:
    ...
    def copy_model(self, model: 'GeneExpressionPredictor') -> 'GeneExpressionPredictor':
        """
    ...
    def load_model(self, path: str, only_backbone: bool = False):
...
def create_model(
    ...
    **kwargs
    ...
    """
    Factory function to create a GeneExpressionPredictor model.
