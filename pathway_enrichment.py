"""
Simple KEGG Pathway Enrichment Analysis using GSEApy
Returns DataFrame of pathway enrichment scores for each cell
"""
...
def get_breast_cancer_pathways() -> List[str]:
    """Get breast cancer relevant KEGG pathway IDs"""
...
def wilcoxon_vectorized(X,ranks, gene_mask):
    ...
    # ----- 1) Compute ranks per column (vectorized via rankdata with axis=0) -----
    # rankdata produces float64 ranks with average-rank for ties
    #ranks = rankdata(X, axis=0, method='average')

    # ----- 2) Sum ranks of genes in gene set -----
    # mask broadcasts to (n_genes, n_cells)
    R1 = np.sum(ranks[gene_mask, :], axis=0)

    # ----- 3) Compute U statistics for group 1 -----
    U = R1 - n1 * (n1 + 1) / 2.0

    # ----- 4) Normal approximation for p-values -----
    mean_U = n1 * n2 / 2.0
    std_U = np.sqrt(n1 * n2 * (n_genes + 1) / 12.0)

    z = (U - mean_U) / std_U
    pvals = norm.sf(z)  # two-sided
